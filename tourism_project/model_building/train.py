...
# Define file paths within the Hugging Face dataset
Xtrain_filename = "Xtrain.csv"
Xtest_filename = "Xtest.csv"
ytrain_filename = "ytrain.csv"
ytest_filename = "ytest.csv"

# Download files locally using hf_hub_download before reading with pandas
Xtrain_local_path = hf_hub_download(repo_id=repo_id, filename=Xtrain_filename, repo_type="dataset")
Xtest_local_path = hf_hub_download(repo_id=repo_id, filename=Xtest_filename, repo_type="dataset")
ytrain_local_path = hf_hub_download(repo_id=repo_id, filename=ytrain_filename, repo_type="dataset")
ytest_local_path = hf_hub_download(repo_id=repo_id, filename=ytest_filename, repo_type="dataset")

# Read data from locally downloaded files
Xtrain = pd.read_csv(Xtrain_local_path)
Xtest = pd.read_csv(Xtest_local_path)
ytrain = pd.read_csv(ytrain_local_path).squeeze() # Ensure ytrain is a Series
ytest = pd.read_csv(ytest_local_path).squeeze()   # Ensure ytest is a Series

# Create Column Transformer with 2 types of Transformer

cat_features = Xtrain.select_dtypes(include='object').columns
num_features = Xtrain.select_dtypes(exclude='object').columns
...
# Define base XGBoost model with class weight
...
with mlflow.start_run():
    ...
    # The repo_id is already defined above
    repo_type = "model"
    ...
    # create_repo("travel-package-purchase-model", repo_type="model", private=False)
    api.upload_file(
